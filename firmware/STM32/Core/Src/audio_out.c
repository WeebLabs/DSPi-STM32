...
/* ---- Dithered 24-bit output requantization ----
 * The SAI carries 24-bit samples, so the float DSP output has to be
 * requantized. A bare truncating cast leaves the quantization error
 * correlated with the signal (a small DC bias + low-level harmonic
 * distortion). Instead we round-to-nearest with TPDF dither (±1 LSB
 * triangular), which makes the residual a signal-independent ~−135 dBFS
 * white floor with no distortion. No noise shaping: at 24 bits that floor
 * is already below any DAC's analog noise, so shaping would only relocate
 * inaudible noise at real CPU cost across the eight output channels.
 *
 * Dither source: one shared xorshift32 stream advanced twice per sample
 * (two independent uniforms → one triangular draw). A single stream across
 * the channels keeps them mutually decorrelated. Single-core: only
 * fill_half touches the state. Exact digital silence (v == 0) is passed
 * through as 0 so muted/disabled outputs stay truly black instead of
 * carrying a dither floor. */
static uint32_t out_dither_rng = 0x1234567u;   /* xorshift32 state, must be nonzero */

static inline uint32_t f2s24_tpdf(float v) {
    if (v == 0.0f) return 0u;                  /* keep true silence silent */
    uint32_t x = out_dither_rng;
    x ^= x << 13; x ^= x >> 17; x ^= x << 5;
    float u1 = (float)x * (1.0f / 4294967296.0f);   /* uniform [0,1) */
    x ^= x << 13; x ^= x >> 17; x ^= x << 5;
    float u2 = (float)x * (1.0f / 4294967296.0f);
    out_dither_rng = x;
    /* Round-to-nearest with the TPDF dither folded in. Done as add-±0.5-then-
     * truncate rather than lrintf(): the (int32_t) cast is a single VCVT
     * (toward zero), whereas lrintf() is an out-of-line libcall on newlib.
     * With dither present the half-way tie-break rule is irrelevant. */
    float y = v * FLOAT_TO_24 + (u1 - u2);
    int32_t q = (int32_t)(y + (y >= 0.0f ? 0.5f : -0.5f));
    if (q >  8388607) q =  8388607;            /* clamp to +2^23 − 1 */
    else if (q < -8388608) q = -8388608;       /* clamp to −2^23      */
    return (uint32_t)q & 0x00FFFFFFu;          /* 24-bit 2's-complement field */
}
...
static void fill_half(int32_t *dst_a, int32_t *dst_b,
    ...
    for (uint32_t k = 0; k < AUDIO_FRAMES_HALF; ++k) {
        ...
        /* Round + TPDF-dither to 24 bits via f2s24_tpdf(), which also masks
         * to the 24-bit field. The mask is essential in SPDIF mode: the SAI
         * reads DR[24]=V, [25]=U, [26]=C as IEC 60958 metadata, so letting
         * int32 sign-extension push 1s into those bits would flag every
         * negative sample Invalid (V=1) and randomise U/C — audibly distorted
         * on the negative half of every cycle. Masking keeps clean 24-bit
         * 2's-complement (e.g. -1 = 0xFFFFFF) with V/U/C = 0 (valid, no user
         * data, all-zero channel-status block that receivers tolerate). For
         * I2S the upper bits are ignored (DataSize=24). P (parity, DR[27]) is
         * auto-generated by the SAI regardless. */
        dst_a[2*k + 0] = f2s24_tpdf(o[0]);
        dst_a[2*k + 1] = f2s24_tpdf(o[1]);
        dst_b[2*k + 0] = f2s24_tpdf(o[2]);
        dst_b[2*k + 1] = f2s24_tpdf(o[3]);
        dst_c[2*k + 0] = f2s24_tpdf(o[4]);
        dst_c[2*k + 1] = f2s24_tpdf(o[5]);
        dst_d[2*k + 0] = f2s24_tpdf(o[6]);
        dst_d[2*k + 1] = f2s24_tpdf(o[7]);
    }
